...
def de(fun, bounds, mut=0.8, crossp=0.9, population=100, its=3000):
	dimensions = len(bounds)
	population = np.random.rand(population, dimensions)
	min_b, max_b = np.asarray(bounds).T
	diff = np.fabs(min_b - max_b)
	population_denorm = min_b + population * diff
	fitness = np.asarray([fun(ind) for ind in population_denorm])
...
output = list(pso(weierstrass, its = 200, populationSize = 100, c1 = 2.05, c2 = 2.05, w = 0.65, wdamp = 0.995, dimensions=2))
...
# plt.plot(output, label='d={}'.format(d))
# plt.legend()
# plt.show()
# plt.savefig('ackley2')
